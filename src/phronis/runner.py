...
def run_training(console: Console, config_path: str, output_name: str, target_loss: float = None) -> bool:
    ...
    with Live(
        Panel("", title=f"[bold rgb(168,85,247)]Training - {output_name}[/]", border_style="rgb(168,85,247)"),
        console=console,
        ...
        while not done.is_set():
            ...
            live.update(
                Panel(
                    content,
                    title=f"[bold rgb(168,85,247)]Training - {output_name}[/]",
                    border_style="rgb(168,85,247)",
                )
            )
            time.sleep(0.12)
